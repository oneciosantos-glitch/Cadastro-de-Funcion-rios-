...
Seguranca: defina uma senha de acesso em .streamlit/secrets.toml
(`senha = "suaSenha"`) ou na variavel de ambiente SISTEMA_SENHA.
Sem senha o link fica aberto para qualquer pessoa que o tiver.
"""
...
def senha_configurada():
    try:
        if st.secrets.get("senha"):
            return str(st.secrets["senha"])
    except Exception:
        pass
    return os.environ.get("SISTEMA_SENHA") or ""
...
def exigir_login():
    ...
    if not esperada:
        ...
        st.warning(
            "Acesso sem senha: qualquer pessoa com o link pode ver e alterar "
            "os cadastros. Configure uma senha em `.streamlit/secrets.toml` "
            "(`senha = 'suaSenha'`) ou na variavel SISTEMA_SENHA.",
            icon="\u26A0\uFE0F")
        return True
    if st.session_state.get("logado"):
        return True

    estilo.cabecalho("Cadastro de Funcionarios", "Acesse o sistema")
    with st.form("login"):
...
def pagina_cadastro(banco):
    ...
    with st.form("cadastro", clear_on_submit=False):
        ...
        loja = c1.selectbox("\U0001F3EA Loja", [""] + combos["loja"],
                            index=indice(combos["loja"],
                                         reg["loja"] if reg else None))
        situacao = c2.selectbox(
            "\U0001F4CB Situacao", [""] + combos["situacao"],
            index=indice(combos["situacao"],
                         reg["situacao"] if reg else "Ativo"))
        cargo = c3.selectbox("\U0001F4BC Cargo", [""] + combos["cargo"],
                             index=indice(combos["cargo"],
                                          reg["cargo"] if reg else None))
...
def main():
    ...
    banco = get_banco()
    estilo.marca()
    with st.sidebar.expander("\U0001F511 Acesso"):
        if senha_configurada():
            st.caption("Sistema protegido por senha")
            if st.button("Sair"):
                st.session_state["logado"] = False
                st.rerun()
        else:
            st.caption("Sem senha - aberto")
