...
def pagina_cadastro(banco):
    ...
    def _default(campo, reg_val=None):
        ...
        if ocr_campos and campo in ocr_campos and ocr_campos[campo]:
            return ocr_campos[campo]
        if reg_val is not None:
            return reg_val
        return ""

    _def_loja = _default("loja", reg["loja"] if reg and reg.get("loja") else "")
    _def_cargo = _default("cargo", reg["cargo"] if reg and reg.get("cargo") else "")
    _def_sit = _default("situacao", reg["situacao"] if reg and reg.get("situacao") else "Ativo")
    _def_matricula = _default("matricula", reg["matricula"] if reg else banco.proxima_matricula())
    _def_nome = _default("nome", reg["nome"] if reg else "")
    _def_rg = _default("rg", (reg["rg"] or "") if reg else "")
    _def_cpf = _default("cpf", (reg["cpf"] or "") if reg else "")
    _def_telefone = _default("telefone", (reg["telefone"] or "") if reg else "")
    _def_admissao = None
    if ocr_campos and ocr_campos.get("admissao"):
        try:
            _def_admissao = date.fromisoformat(ocr_campos["admissao"])
        except (ValueError, TypeError):
            _def_admissao = None
    if _def_admissao is None:
        _def_admissao = cal.parse_data(reg["admissao"]) if reg else date.today()
    _def_obs = _default("observacao", (reg["observacao"] or "") if reg else "")
    _def_exp = ""
    if ocr_campos and ocr_campos.get("experiencia_dias"):
        _def_exp = f"{ocr_campos['experiencia_dias']} dias"
    elif reg and reg["experiencia_dias"]:
