...
def pagina_config(banco):
    ...
    if valores:
        ...
        for v in valores:
            ...
            btn_key = f"exc_combo_{tipo}_{v}"
            if cv2.button("\U0001F5D1 Excluir", key=btn_key):
                ok = banco.remover_combo(tipo, v)
                if ok:
                    st.success(f"\u2705 '{v}' removido de {tipo}.")
                    st.rerun()
                else:
                    st.error(f"\u274C '{v}' nao pode ser removido: esta em uso por um ou mais funcionarios.")
    else:
...
def pagina_dashboard(banco):
    ...
    k8.metric("\U0001F7E1 Alerta 4 meses", ev["ferias_alerta_4m"])
