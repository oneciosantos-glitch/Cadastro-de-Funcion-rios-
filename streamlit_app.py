...
def pagina_eventos(banco):
    ...
    c7.metric("\U0001F7E1 Ferias 4m", len(alerta_4m))
...
def pagina_config(banco):
    ...
    # Tabela com botoes de excluir – usar callback para acao confiavel
    valores = banco.listar_combo(tipo)
    if valores:
        st.markdown("**Opcoes existentes:**")
        # Verificar se algum botao de exclusao foi clicado (logica antes dos botoes)
        for idx, v in enumerate(valores):
            btn_key = f"exc_combo_{tipo}_{idx}"
            if st.session_state.get(btn_key, False):
                ok = banco.remover_combo(tipo, v)
                if ok:
                    st.session_state["_combo_msg"] = (
                        f"\u2705 '{v}' removido de {tipo}.")
                    st.session_state["_combo_erro"] = None
                else:
                    st.session_state["_combo_msg"] = None
                    st.session_state["_combo_erro"] = (
                        f"\u274C '{v}' nao pode ser removido: "
                        "esta em uso por um ou mais funcionarios.")
                st.session_state[btn_key] = False
                st.rerun()
        # Exibir mensagem de feedback (persiste apos rerun)
        if st.session_state.get("_combo_msg"):
            st.success(st.session_state["_combo_msg"])
            st.session_state["_combo_msg"] = None
        if st.session_state.get("_combo_erro"):
            st.error(st.session_state["_combo_erro"])
            st.session_state["_combo_erro"] = None
        # Renderizar os botoes (apos a logica, conforme recomendacao Streamlit)
        for idx, v in enumerate(valores):
            cv1, cv2 = st.columns([3, 1])
            cv1.markdown(f"\U0001F4CB {v}")
            btn_key = f"exc_combo_{tipo}_{idx}"
            cv2.button("\U0001F5D1 Excluir", key=btn_key)
    else:
