...
def pagina_cadastro(banco):
    ...
    # Determinar dias de experiencia atuais (do form ou do registro)
    _exp_dias = None
    ...
    if _exp_dias and admissao:
        _prazos = cal.contrato_experiencia(admissao, _exp_dias).prazos_intermediarios
        if _prazos:
            st.divider()
            st.subheader("\U0001F4C5 Prazos do contrato de experiencia")
            for _prazo_dias, _prazo_label in _prazos:
                estilo.cartao_prazo_experiencia(
                    _prazo_dias, admissao, _prazo_label)
